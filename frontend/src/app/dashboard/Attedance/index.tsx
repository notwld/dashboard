...
const Attendance = () => {
  ...
  const fetchAttendance = useCallback(async () => {
    try {
      const response = await fetch(`${baseurl}/attendance/all-attendance`, {
        method: 'POST',
        headers: {
  ...
  const handleLeaveRequest = async () => {
    try {
      const response = await fetch(`${baseurl}/attendance/apply-leave`, {
        method: 'POST',
        headers: {
          "x-access-token": `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  }, [attendance, fetchAttendance, today]);
          ...
          <Card className="w-full h-[450px]">
            ...
            <CardContent className="h-full">
              <div className="grid grid-cols-1 gap-5">
                ...
                <div className="col-span-2 items-end align-bottom justify-end text-center">
            ...
            <CardContent>
              {attendance?.filter((entry) => entry.isOnLeave === true).map((entry, index) => (
                <div key={index} className="flex flex-col gap-2 my-2">
                  <p>
                    You applied for leave on {new Date(entry.date).toLocaleDateString()} which is {entry.leaveStatus?.toLowerCase()}.
                  </p>
                </div>
              ))}
            </CardContent>
